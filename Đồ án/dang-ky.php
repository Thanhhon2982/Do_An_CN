...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (strlen(postInput('password')) < 6) {
        $error['password'] = "Mật khẩu phải có ít nhất 6 kí tự";
    }
    if (!preg_match('/[a-zA-Z]/', postInput('password')) || !preg_match('/[0-9]/',postInput('password'))) {
        $error['password'] = "Mật khẩu phải chứa ít nhất một ký tự chữ và một số";
    }

    if (ctype_alnum(postInput('password'))) {
        $error['password'] = "Mật khẩu phải chứa ít nhất một ký tự đặc biệt";
    }
    if (postInput('phone') == '') {
    ...
    if (empty($error)) {   //câu lênh cho upload file            
