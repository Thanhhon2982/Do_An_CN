...
class Database
{
    ...
    {
        $this->link = mysqli_connect("localhost", "root", "", "nthstore") or die();
    ...
    public function insertvnpay(array $data_vnpay)
    {
        ...
        foreach ($data_vnpay as $field => $value) {
            if (is_string($value)) {
                $values .= "'" . mysqli_real_escape_string($this->link, $value) . "',";
            } else {
                $values .= mysqli_real_escape_string($this->link, $value) . ',';
        ...
        mysqli_query($this->link, $sql) or die("Lỗi truy vấn insertvnpay ----" . mysqli_error($this->link));
        return mysqli_insert_id($this->link);
    }
